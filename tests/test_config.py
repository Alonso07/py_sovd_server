...
def test_configuration():
    ...
    try:
        ...
        print(
            f"   ✓ Gateway config loaded: {gateway_config['gateway']['name']} v{gateway_config['gateway']['version']}"
        )
        ...
        if camera_op:
            ...
            print(
                f"     - Type: {camera_op.get('execution', {}).get('type', 'unknown')}"
            )
            print(
                f"     - Timeout: {camera_op.get('execution', {}).get('timeout', 'unknown')}s"
            )
        else:
        ...
        if engine_faults:
            ...
            for fault in engine_faults[:2]:  # Show first 2
                print(
                    f"     - {fault['id']}: {fault['name']} (Severity: {fault['severity']})"
                )
        else:
        ...
        if engine_modes:
            ...
            for mode in engine_modes:
                print(
                    f"     - {mode['id']}: {mode['name']} (Status: {mode['details']['status']})"
                )
        else:
...
def test_api_endpoints():
    ...
    try:
        ...
        if engine_data and "schema" in engine_data:
            ...
            print(
                f"   ✓ EngineRPM schema: {schema['type']} with {len(schema.get('properties', {}))} properties"
            )
        ...
        if camera_op and "request_payload" in camera_op:
            ...
            for param, details in payload.items():
                print(
                    f"     - {param}: {details.get('type', 'unknown')} (default: {details.get('default', 'none')})"
                )
