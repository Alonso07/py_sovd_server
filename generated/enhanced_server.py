...
def entity_capabilities(entity_path):
    ...
    try:
        ...
        logger.info(f"Looking up entity: {entity_path}")
...
def operations(entity_path):
    ...
    try:
        ...
        for operation_file in operation_files:
            operation_config = config_loader.load_resource_config('operations', operation_file)
            ...
            for op_data in ops:
                operation = Operation(
                    ...
                    capabilities=op_data.get('tags', [])  # Use tags as capabilities
                )
...
def start_operation_execution(entity_path, operation_id):
    ...
    try:
        ...
        logger.info(f"Operation execution started: {entity_path}/{operation_id} (ID: {execution_id})")
    ...
    try:
        ...
        for fault_data in faults_data:
            ...
            fault_ref = FaultReference(
                ...
                fault_translation_id=fault_data['id'],
                severity=1 if fault_data['severity'] == 'critical' else 2 if fault_data['severity'] == 'error' else 3 if fault_data['severity'] == 'warning' else 4,
                status=None  # We'll set this to None for now
            )
            
            # Create Fault object with FaultReference
    ...
    try:
        ...
        for mode_data in modes_data:
            mode = Mode(
                ...
                value=mode_data['id']  # Use id as value
            )
            modes.append(mode)
