...
def main():
    ...
    if not os.path.exists(os.path.join(script_dir, "enhanced_server.py")):
        print(
            "Error: enhanced_server.py not found. Please ensure the file is in the correct location."
        )
        sys.exit(1)
    ...
    if not os.path.exists(os.path.join(script_dir, "config")):
        print(
            "Error: config directory not found. Please ensure configuration files are present."
        )
        sys.exit(1)
    ...
    if not os.path.exists(generated_path):
        print(
            "Error: generated directory not found. Please ensure the generated server is present."
        )
        sys.exit(1)
    ...
    try:
        ...
        subprocess.run(
            [sys.executable, os.path.join(script_dir, "enhanced_server.py")], check=True
        )
